"""
Claude Advisor — LLM-powered analysis integration (generic).
...
def _run_analysis(api_key: str, analysis: dict, dashboard_data: dict) -> dict | None:
    ...
    project_name = dashboard_data.get("project_name", "Unknown Project")
    project_type = dashboard_data.get("project_type", "software")
    frameworks = dashboard_data.get("frameworks", [])
    fw_str = ", ".join(frameworks) if frameworks else "unknown stack"

    section_ids = list(analysis.keys())
    ...
    try:
        ...
        message = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""You are analyzing a {project_type} project called {project_name} built with {fw_str}.
...
Use these section IDs: {', '.join(section_ids)}
